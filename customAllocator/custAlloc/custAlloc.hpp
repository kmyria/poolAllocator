...
class PoolAllocator {
    ...
    private:
        ...
        struct HeadView {
                uintptr_t packed;
                FreeNode* ptr() const { return reinterpret_cast<FreeNode*>(packed & PTR_MASK); }
        ...
        uintptr_t pack(FreeNode* ptr, std::uint64_t tag)
        {
                return (reinterpret_cast<uintptr_t>(ptr) & PTR_MASK)
                    | ((tag << PTR_BITS) & TAG_MASK);
        }

        static constexpr uintptr_t PTR_BITS = 48;
        static constexpr uintptr_t PTR_MASK = (std::uintptr_t { 1 } << PTR_BITS) - 1;
        static constexpr uintptr_t TAG_MASK = ~PTR_MASK;

        alignas(
            std::hardware_constructive_interference_size) std::atomic<uintptr_t> free_list_head_;
        ...
        FreeNode* global_pop_single()
        {
                ...
                do {
                    ...
                    old_packed, new_packed, std::memory_order_acquire, std::memory_order_acquire));
